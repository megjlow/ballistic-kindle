...
void firmata_end(t_firmata* firmata) {
	close(firmata->serial->port_fd);
	free(firmata->serial);
	free(firmata);
}
...
int		firmata_pull(t_firmata *firmata)
{
  ...
  if (r > 0) {
	  while(1) {
		...
		char lastbyte = buffer[r - 1];

		if(lastbyte == 0 || nbytes == 0) {
			break;
		}
	  }
	  firmata_parse(firmata, buff, count);
  }
  return count;
}
...
int		firmata_pinMode(t_firmata *firmata, int pin, int mode)
{
  ...
  buff[1] = 0x25;
  buff[2] = 0x00;
  buff[3] = FIRMATA_SET_PIN_MODE;
  buff[4] = pin;
  buff[5] = mode;
  buff[6] = 0x00;
  printf("Setting pinMode at: %i with value: %i\n", pin, mode);
  res = serial_write(firmata->serial, buff, sizeof(buff));
  return (res);
}
...
int		firmata_digitalWrite(t_firmata *firmata, int pin, int value)
{
  ...
  buff[1] = 0x25;
  buff[2] = 0x00;
  buff[3] = FIRMATA_DIGITAL_MESSAGE | port_num;
    buff[4] = port_val & 0x7F;
    buff[5] = (port_val >> 7) & 0x7F;
    buff[6] = 0x00;
  */
  /*
  ...
  */
  uint8_t buff[7] = { 0x12, 0x25, 0x00, FIRMATA_DIGITAL_MESSAGE | port_num, port_val & 0x7F, (port_val >> 7) & 0x7F, 0x00 };
  res = serial_write(firmata->serial, buff, sizeof(buff));
  return (res);
}
